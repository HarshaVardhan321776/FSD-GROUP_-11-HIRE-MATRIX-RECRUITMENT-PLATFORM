...
const normalizeSmtpPass = () => {
  ...
  // Gmail app passwords are shown in 4-char groups; remove accidental spaces.
  if (host.includes('gmail') && /\s/.test(rawPass)) {
...
const createTransporterFromEnv = () => {
  return nodemailer.createTransport({
    ...
    auth: process.env.SMTP_USER ? { user: process.env.SMTP_USER, pass: normalizeSmtpPass() } : undefined,
  });
};
